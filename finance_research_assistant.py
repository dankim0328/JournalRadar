...
def save_to_notion(papers, analyzed_results):
    ...
    try:
        ...
        page_kor = notion.pages.create(
            ...
            properties={"title": [{"text": {"content": f"Top Finance Journals Weekly Report ({today_str})"}}]}
        )
        
        # 2. ENG Page
        page_eng = notion.pages.create(
            parent={"page_id": NOTION_PAGE_ID},
            properties={"title": [{"text": {"content": f"[ENG] Top Finance Journals Weekly Report ({today_str})"}}]}
        )
