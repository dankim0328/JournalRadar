...
NOTION_PAGE_ID = os.environ.get("NOTION_ACCOUNTING_PAGE_ID", "")
...
def analyze_paper_with_gemini(paper):
    prompt = f"""당신은 세계적인 회계(Accounting) 학술 연구 보조 AI입니다.
아래 회계(Accounting) 탑 저널 논문 정보를 바탕으로 다음 3가지를 분석해 주세요.
...
def # CSV/MD 파일 생성 생략:
    ...
    csv_filename = f"accounting_papers_{today_str}.csv"
...
def save_to_notion(papers, analyzed_results):
    ...
    try:
        ...
        page_kor = notion.pages.create(
            ...
            properties={"title": [{"text": {"content": f"Top Accounting Journals Weekly Report ({today_str})"}}]}
        )
        
        # 2. ENG Page
        page_eng = notion.pages.create(
            parent={"page_id": NOTION_PAGE_ID},
            properties={"title": [{"text": {"content": f"[ENG] Top Accounting Journals Weekly Report ({today_str})"}}]}
        )
